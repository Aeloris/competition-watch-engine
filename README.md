...
## 当前进度：Phase 8（Eval-Harness 回放评测 + 人工放行闭环 + 发布视图 + 简单面板）
...
- **FastAPI 0.8.0** `app/routers`：`POST /tasks`（建 + 同步执行 → 201 终态）、`GET /tasks`、`GET /tasks/{id}`、
  `GET /tasks/{id}/report`（各 done run 从 pipeline json 合并全量周报 draft）、`GET /tasks/{id}/published`（Phase 8 发布视图）、
  `GET /inbox`（`?status=pending|all|resolved`，缺省 pending = P7 语义）、`POST /inbox/resolve`（Phase 8 人工定案）、
  `GET /alerts`、`POST /alerts/hook`（手动投递演练；非法 event_type 422）、`GET /panel` + `GET /panel/task/{id}`（Phase 8 面板）。
...
- **全绿 165**（123 基线 + service 21 + eval 15 + panel 6：`tests/test_eval.py` + `test_panel_api.py`）；
...
- **诚实边界**：Webhook 仍是占位壳（不真发 HTTP），周报**对外真推送无渠道**——发布闭环止于 published_view（HTTP 查询态
  ...
  为门卫级非图级）；定时只在进程活着时生效（单进程长跑定位）；POST /tasks 同步执行（演示直观，超长任务队列留待接）。
  README2 里的目标指标（告警延迟/覆盖率等）是**设计占位、从未实测，不上简历**——本期实测口径只有上面测试锁死的数字。
  细节见 [`docs/service.md`](docs/service.md) + [`docs/eval.md`](docs/eval.md)。

```bash
uv sync
uv run pytest            # 全绿（165）
...
docs/   architecture.md 骨架现状 · schema.md · collectors.md · memory.md · orchestrator.md
        · analyst_writer.md · reviewer.md · service.md · eval.md（Phase 8）
tests/  冒烟 / mock / 数据层 / 采集层 / 去重 / diff / 编排 / 分级 / 周报 / 门卫
        / service_runtime / service_api / eval / panel_api
```
...
harness / run。HTTP 层在 `app/routers`（薄，全逻辑在 service/eval）。`reporter/publisher.py` 仍占位（周报**对外**真推送
属接入期；发布闭环的查询态已由 published_view 承担）。`LICENSE` = MIT（Copyright (c) 2026 qiaosheng）。
